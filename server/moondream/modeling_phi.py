...
class RotaryEmbedding(nn.Module):
    ...
    def __init__(
        ...
        self._update_cos_sin_cache(
            max_position_embeddings, device=device, dtype=torch.float16
        )
    ...
    def _calculate_scale(self, dim, scale_base, device):
        return (
            (
                (
                    torch.arange(0, dim, 2, device=device, dtype=torch.float16)
                    + 0.4 * dim
                )
                / (1.4 * dim)
            )
    ...
    def _compute_inv_freq(self, device: Optional[str] = None) -> torch.FloatTensor:
        return 1.0 / (
            self.base
            ** (
                torch.arange(0, self.dim, 2, device=device, dtype=torch.float16)
                / self.dim
            )
        )
    ...
    def _update_cos_sin_cache(
        ...
        t = torch.arange(
            seqlen,
            device=device,
            dtype=torch.float16 if self.pos_idx_in_fp32 else self.inv_freq.dtype,
        )
        inv_freq = (
            self._compute_inv_freq(device=device)
            if self.pos_idx_in_fp32 and self.inv_freq.dtype != torch.float16
            else self.inv_freq
        )
...
class SelfAttention(nn.Module):
    ...
    def forward(
        ...
        scores = (
            torch.einsum("bthd,bshd->bhts", q.to(torch.float16), k.to(torch.float16))
            * scale
        )
...
class CausalLMHead(nn.Module):
    ...
    def forward(self, hidden_states):
        return self.linear(self.ln(hidden_states)).to(torch.float16)
